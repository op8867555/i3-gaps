...
static int json_int(void *ctx, long long val) {
    ...
    if (parsing_gaps) {
        ...
        else if (strcasecmp(last_key, "outer.left") == 0)
            json_node->gaps.outer.left = val;
        else if (strcasecmp(last_key, "outer.top") == 0)
            json_node->gaps.outer.top = val;
        else if (strcasecmp(last_key, "outer.right") == 0)
            json_node->gaps.outer.right = val;
        else if (strcasecmp(last_key, "outer.bottom") == 0)
